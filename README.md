...
리포지토리 루트에서 C++ 컴파일러와 CMake가 `PATH`에 있어야 합니다. `packages.txt`에 맞춰 Linux에서는 `build-essential`, `cmake`, `git`, `python3-dev`를 설치한 뒤:
...
├── requirements.txt          # pip: 현재 프로젝트 루트 설치 (`pip install -r requirements.txt`)
