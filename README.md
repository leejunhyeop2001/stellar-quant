...
**대시보드 코드 고정값** (`python/dashboard.py` 상단 `FIXED_*`): 터미널 경로 **10,000,000**, 예측 기간 **1.0년**, 팬 샘플 **8,000** 경로, 시간 스텝 **252**, 스레드 **0**(자동), Merton 점프(시장 중립에 가깝게) **λ=1.0, μ_J=-0.02, σ_J=0.05**; 시세 실패 시 GBM 폴백 **S0=250, μ=0.10, σ=0.40**. 리스크 등급은 `data_utils.compute_risk_metrics`의 **정규화 Risk Score** `(VaR95/S0)/(σ√T)` 와 종료 로그수익 **초과첨도** 가중으로 산출합니다.
